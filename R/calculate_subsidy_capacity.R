  ...
  if (!is.null(df_prek)) {
    df <- df_prek %>% 
      ...
      dplyr::mutate(infant = FALSE,
                    ...
                    quality_desc = dplyr::case_when(quality == TRUE ~ "TEA Accountability Rating"),
                    sub_trs_provider = ifelse(quality == TRUE, TRUE, FALSE),
                    sub_trs4_provider = ifelse(quality == TRUE, TRUE, FALSE),
                    download_date = NA) %>% 
